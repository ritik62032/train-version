...
console.log('================ CHAT MICROSERVICE LOADED ================');

router.post('/', async(req, res) => {
    console.log('POST /api/chat received request with body:', JSON.stringify(req.body, null, 2));

    // Try all possible sources for the API key, with hardcoded fallback
    const API_KEY = process.env.GEMINI_API_KEY || process.env.REACT_APP_GEMINI_API_KEY || HARDCODED_API_KEY;

    console.log(`[CHAT API] Using API Key (first 5 chars): ${API_KEY.substring(0, 5)}`);
    ...
    try {
        ...
        // Use the Gemini API with the correct model name (gemini-pro)
